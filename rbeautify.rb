...
if __FILE__ == $0
  ...
  op = OptionParser.new{|o|
    o.banner = <<-BANNER
rbeautify [OPTION]... [FILE]...
rbeautify [OPTION]... -

Options are:
    BANNER
    o.on('-i', '--tab-size INTEGER', Integer){|ts| options[:tab_size] = ts }
    o.on('-t', '--tab-string STRING'){|ts| options[:tab_string] = ts }
  ...
  beautify = RBeautify.new(options)

  error = false

  ARGV.each do |path|
    error = (beautify.beautify_file(path)) ? true : error
  end
